...
"""Import a validator script by filename.

The validators are hyphenated CLI entry points (`check-nfc.py`), which is
...
def load(script_filename: str) -> ModuleType:
    module_name = f"vauchi_locales_{script_filename.removesuffix('.py').replace('-', '_')}"
